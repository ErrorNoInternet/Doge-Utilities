...
async def on_message(message):
	try:
		...
		if message.content.startswith(prefix + "tests"):
			if getCooldown(message.author.id, "tests") > 0:
				cooldownString = generateCooldown("tests", getCooldown(message.author.id, "tests"))
...
:grey_question: `Image Library`
			...
			for test in tests:
				...
				if name == "Code Integrity":
					...
					try:
						...
						description = description.replace(test, test.replace(":grey_question:", ":red_square:"))
				elif name == "Help Pages":
					try:
						for i in range(20):
							pages = {}; currentPage = 1; pageLimit = 10; currentItem = 0; index = 1; pageArguments = False
							for commandName in variables.commands:
								currentPage = commandName
								for command in variables.commands:
									commandName = command.split(" ")[0]
									if commandName == currentPage:
										commandArguments = command.split(" "); commandArguments.pop(0)
										commandArguments = ' '.join(commandArguments)
										if commandArguments == "":
											commandArguments = "None"
										commandExample = prefix + parseVariables(command)
										command = f"Command: `{prefix}{commandName}`\nAdditional arguments: `{commandArguments}`\nUsage example: `{commandExample}`\n\n**{variables.commands[command]}**"
						description = description.replace(test, test.replace(":grey_question:", ":green_square:"))
					except:
						description = description.replace(test, test.replace(":grey_question:", ":red_square:"))
		...
		if message.content.startswith(prefix + "status"):
			if getCooldown(message.author.id, "status") > 0:
				cooldownString = generateCooldown("status", getCooldown(message.author.id, "status"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
			...
			embed.add_field(name="Command Count", value="```" + str(len(variables.commands)) + "```")
		...
		if message.content.startswith(prefix + "version"):
			if getCooldown(message.author.id, "version") > 0:
				cooldownString = generateCooldown("version", getCooldown(message.author.id, "version"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "setup-banned"):
			...
			if getCooldown(message.author.id, "setup") > 0:
				cooldownString = generateCooldown("setup", getCooldown(message.author.id, "setup"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "setup-muted"):
			if message.author.guild_permissions.manage_roles or message.author.id in variables.permissionOverride:
				pass
			else:
				await message.channel.send("You do not have permission to use this command!"); return
		...
		if message.content.startswith(prefix + "random"):
			if getCooldown(message.author.id, "random") > 0:
				cooldownString = generateCooldown("random", getCooldown(message.author.id, "random"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "execute;"):
			if message.author.id == variables.botOwner:
				try:
					...
					output = output.replace(os.getenv("TOKEN"), "<token>").replace("`", "\`")
		...
		if message.content.startswith(prefix + "disconnect-members"):
			if getCooldown(message.author.id, "disconnect-members") > 0:
				cooldownString = generateCooldown("disconnect-members", getCooldown(message.author.id, "disconnect-members"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "suggest"):
			if getCooldown(message.author.id, "suggest") > 0:
				cooldownString = generateCooldown("suggest", getCooldown(message.author.id, "suggest"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "autorole"):
			if getCooldown(message.author.id, "autorole") > 0:
				cooldownString = generateCooldown("autorole", getCooldown(message.author.id, "autorole"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "shards"):
			if getCooldown(message.author.id, "shards") > 0:
				cooldownString = generateCooldown("shards", getCooldown(message.author.id, "shards"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "lookup"):
			if getCooldown(message.author.id, "lookup") > 0:
				cooldownString = generateCooldown("lookup", getCooldown(message.author.id, "lookup"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
			...
			if "10013" not in str(response):
				...
				if response['banner'] != None:
					if response['banner'].startswith("a_"):
						bannerURL = f"https://cdn.discordapp.com/banners/{response['id']}/{response['banner']}.gif?size=512"
					else:
						bannerURL = f"https://cdn.discordapp.com/banners/{response['id']}/{response['banner']}.webp?size=512"
		...
		if message.content.startswith(prefix + "permissions") or message.content.startswith(prefix + "perms"):
			if getCooldown(message.author.id, "permissions") > 0:
				cooldownString = generateCooldown("permissions", getCooldown(message.author.id, "permissions"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "raid-protection"):
			if message.author.guild_permissions.administrator or message.author.id in variables.permissionOverride:
				try:
		...
		if message.content.startswith(prefix + "epoch-date"):
			arguments = message.content.split(" ")
			if len(arguments) > 1:
		...
		if message.content.startswith(prefix + "date-epoch"):
			arguments = message.content.split(" ")
			if len(arguments) > 1:
				arguments.pop(0); text = " ".join(arguments)
				try:
		...
		if message.content.startswith(prefix + "hash"):
			if getCooldown(message.author.id, "hash") > 0:
				cooldownString = generateCooldown("hash", getCooldown(message.author.id, "hash"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "base64"):
			if getCooldown(message.author.id, "base64") > 0:
				cooldownString = generateCooldown("base64", getCooldown(message.author.id, "base64"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "binary"):
			if getCooldown(message.author.id, "binary") > 0:
				cooldownString = generateCooldown("binary", getCooldown(message.author.id, "binary"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "calculate"):
			if getCooldown(message.author.id, "calculate") > 0:
				cooldownString = generateCooldown("calculate", getCooldown(message.author.id, "calculate"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "clear"):
			if getCooldown(message.author.id, "clear") > 0:
				cooldownString = generateCooldown("clear", getCooldown(message.author.id, "clear"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "wide"):
			if getCooldown(message.author.id, "wide") > 0:
				cooldownString = generateCooldown("wide", getCooldown(message.author.id, "wide"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "spoiler"):
			if getCooldown(message.author.id, "spoiler") > 0:
				cooldownString = generateCooldown("spoiler", getCooldown(message.author.id, "spoiler"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "cringe"):
			if getCooldown(message.author.id, "cringe") > 0:
				cooldownString = generateCooldown("cringe", getCooldown(message.author.id, "cringe"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "reverse"):
			if getCooldown(message.author.id, "reverse") > 0:
				cooldownString = generateCooldown("reverse", getCooldown(message.author.id, "reverse"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "corrupt"):
			if getCooldown(message.author.id, "corrupt") > 0:
				cooldownString = generateCooldown("corrupt", getCooldown(message.author.id, "corrupt"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "color") or message.content.startswith(prefix + "colour"):
			if getCooldown(message.author.id, "color") > 0:
				cooldownString = generateCooldown("color", getCooldown(message.author.id, "color"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
		...
		if message.content.startswith(prefix + "time"):
			if getCooldown(message.author.id, "time") > 0:
				cooldownString = generateCooldown("time", getCooldown(message.author.id, "time"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return
			...
			if len(arguments) > 1:
				...
				await message.channel.send(f"The syntax is `{prefix}corrupt <timezone>`")
			addCooldown(message.author.id, "time", 3)

		if message.content.startswith(f"{prefix}h "):
			message.content = message.content.replace(f"{prefix}h ", f"{prefix}help ")
		if message.content.startswith(prefix + "help") or message.content == prefix + "h":
			if getCooldown(message.author.id, "help") > 0:
				cooldownString = generateCooldown("help", getCooldown(message.author.id, "help"))
				embed = discord.Embed(title="Command Cooldown", description=cooldownString, color=variables.embedColor)
				await message.channel.send(embed=embed); return

			pages = {}; currentPage = 1; pageLimit = 10; currentItem = 0; index = 1; pageArguments = False
			try:
				currentPage = int(message.content.split(prefix + "help ")[1])
				pageArguments = True
			except:
				try:
					currentPage = message.content.split(prefix + "help ")[1]
				except:
					pageArguments = True
					pass
			if pageArguments:
				for command in variables.commands:
					temporaryText = f"`{prefix}{command}` - {variables.commands[command]}\n"
					if index > pageLimit:
						index = 0
						currentItem += 1
					try:
